...
import type { ShellLoadRequest, WorkflowSnapshot, ConfigSubmission } from './types/canon';
import type { ConsoleServices } from './services/console-services';

const request: ShellLoadRequest = {
  user: {
    id: 'operator-42',
    role: 'operator',
    team: 'amr-ops',
...
function bootstrapConsole(services: ConsoleServices) {
  ...
  services.history.persistDefaults(request.user.id, submission);
  ...
  const baseSnapshot: WorkflowSnapshot = {
    ...
    stages: [
      ...
      },
      {
        ...
        status: 'idle',
        dependencies: [
          { id: 'briefing', label: 'Brief ready', status: 'blocked', blockers: ['Await brief approval'] },
        ],
        artifacts: [],
      },
      {
        ...
        status: 'idle',
        dependencies: [
          { id: 'analysis', label: 'Analysis complete', status: 'blocked', blockers: ['Analysis pending'] },
        ],
        artifacts: [],
      },
    ],
...
ReactDOM.createRoot(document.getElementById('root') as HTMLElement).render(
  <React.StrictMode>
    <Shell request={request} bootstrap={bootstrapConsole} />
  </React.StrictMode>,
);
